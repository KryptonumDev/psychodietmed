...
export default function Slider({ items }) {
  ...
  return (
    ...
    >
      {items.map(({ text, author, after, before, boldText }, index) => (
        <SwiperSlide key={index}>
          <div key={author.name + index} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={(after?.mediaItemUrl && before?.mediaItemUrl) ? `${styles.slide}` : `${styles.wide} ${styles.slide}`} >
