...
export default function Hero({ data: { logo, text, image, comment } }) {
  return (
    <section className={styles.wrapper}>
      <div>
        <Image loading='eager' className={styles.logo} aspectRatio={true} src={logo.mediaItemUrl} alt={logo.altText} width={logo.mediaDetails.width} height={logo.mediaDetails.height} />
        <h1>{text}</h1>
      </div>
      <div className={styles.imageWrapper}>
        ...
        <div className={styles.comment}>
          <div className={styles.avatar}>
            <Image loading='eager' src={comment.avatar.mediaItemUrl} alt={comment.avatar.altText || ''} width={comment.avatar.mediaDetails.width} height={comment.avatar.mediaDetails.height} />
            <p>{comment.name}</p>
          </div>
